...
def test_rest_execution(service, test_case, test_filename, generate):
    ...
    if "check_error" in test_case:
        ...
        assert wait_for_pipeline_status(instance_url, "ERROR", states_negative, state_transition_timeout), "Pipeline did not error"
    else:
        state_transition_timeout = float(test_case["check_running"]["timeout"])
        assert wait_for_pipeline_status(instance_url, "RUNNING", states, state_transition_timeout), "Pipeline did not start"
        ...
        if "abort" in test_case:
            ...
            assert wait_for_pipeline_status(instance_url, "ABORTED", states, state_transition_timeout), "Pipeline did not abort"
        else:
            assert wait_for_pipeline_status(instance_url, "COMPLETED", states, state_transition_timeout), "Pipeline did not complete"
