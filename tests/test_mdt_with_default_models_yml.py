...
def test_mdt_with_default_models_yml(capfd):
    ...
    model_download_tool_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                            '../tools/model_downloader')
    model_yml_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                  '../models/models.list.yml')
    # Run the tool
    results = subprocess.run(['python3', model_download_tool_path, '--model-list', model_yml_path,
                              '--output-dir', workdir_path.name, '--force'], check=False)
    ...
    if results.returncode != 0:
        ...
        assert 'Necessary tools needed from Intel(R) distribution of '\
               'OpenVINO(TM) Toolkit not found' in captured.out
    else:
