...
class FrameInfo:
    ...
    def process_frame(self, frame: VideoFrame, threshold: float = DETECT_THRESHOLD) -> bool:
        ...
        while self.json_objects:
            ...
            #   If frame has a higher pts then the timestamp at the head of the list, pop the head of the list for being outdated
            #   If frame has a lower pts then the timestamp at the head of the list, its still possible for the timestamp to come up, so break
            ...
            for indv_object in detected_objects:
                frame.add_region(
                    ...
                    indv_object["detection"]["bounding_box"]["x_max"] - indv_object["detection"]["bounding_box"]["x_min"],
                    indv_object["detection"]["bounding_box"]["y_max"] - indv_object["detection"]["bounding_box"]["y_min"],
