...
Video Analytics Serving simplifies the deployment and use of hardware optimized video analytics pipelines. It offers developers a simple way to create RESTful APIs  to start, stop, enumerate and customize pre-defined pipelines using either [GStreamer](docs/gstreamer.md)
...
## Interfaces
The following table shows the Video Analytics Serving REST API. For more details on internal operation please the [pipeline documentation](docs/pipeline.md).
...
|/pipelines/audio_detection/1|Audio Detection|curl localhost:8080/pipelines/audio_detection/1 -X POST -H 'Content-Type: application/json' -d '{ "source": { "uri": "https://github.com/opencv/gst-video-analytics/blob/preview/audio-detect/samples/gst_launch/audio_detect/how_are_you_doing.wav?raw=true", "type": "uri" }, "destination": { "type": "file", "path": "/tmp/results2.txt", "format":"json-lines"}}'|{"channels":1,"events":[{"detection":{"confidence":1.0,"label":"Speech","label_id":53,"segment":{"end_timestamp":2200000000,"start_timestamp":1200000000}},"end_timestamp":2200000000,"event_type":"Speech","start_timestamp":1200000000}],"rate":16000}|

More details on pipeline definition and operation can be found in the [documentation](docs/pipeline.md).
